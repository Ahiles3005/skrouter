 ...
 */
 ...
 foreach ($request->getPostList() as $k => $arFileData)
 ...
 if(!empty($_POST)){
	...
	// $arParams["USE_CAPTCHA"] = "Y"; 
	 ...
	 // file_put_contents("/home/c/cncrouter/bitrix/public_html/post31.log", print_r(array('$_POST' => $_POST, date("Y-m-d H:i:s")." \n\n",     '$_SERVER[REQUEST_URI]' => $_SERVER['REQUEST_URI']), true), FILE_APPEND);
...
$codeFileFields =  is_array($_POST["codeFileFields"]) ? count($_POST["codeFileFields"]) : 0;
    ...
    if ($arParams["USE_CAPTCHA"]) {
        if ($arParams["CAPTCHA_TYPE"] == "recaptcha") // Google reCAPTCHA v2
        {
            ...
            $answers = json_decode($text, true);
			 ...
			 // file_put_contents("/home/c/cncrouter/bitrix/public_html/post41.log", print_r(array('$_POST' => $_POST, date("Y-m-d H:i:s")." \n\n", '$answers' => $answers), true), FILE_APPEND);
        ...
        } elseif ($arParams["CAPTCHA_TYPE"] == "recaptchav3") // Google reCAPTCHA v3
        {
            ...
            $answers = json_decode($text, true);
			 // file_put_contents("/home/c/cncrouter/bitrix/public_html/post51.log", print_r(array('$_POST' => $_POST, date("Y-m-d H:i:s")." \n\n", '$answers' => $answers), true), FILE_APPEND);
