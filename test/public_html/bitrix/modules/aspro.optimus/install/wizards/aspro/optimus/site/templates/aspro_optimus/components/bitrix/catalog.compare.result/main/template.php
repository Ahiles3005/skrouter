...
<div class="bx_compare" id="bx_catalog_compare_block">
	...
	<div class="catalog-compare swipeignore"<?=$bDelCookie ? ' data-delcookie' : '' ?>>
		<?if ($bUseGroups && count($arResult['SECTIONS']) > 1):?>
			<div class="tabs arrow_scroll tabs--in-section compare-sections__tabs tab_slider_wrapp">
				<ul class="nav nav-tabs tabs">
					<?foreach ($arResult['SECTIONS'] as $arSection):?>
						<li class="compare-sections__tab-item <?=$arSection['ID'] === $activeTabId ? 'cur active' : '' ?>">
							<span class="compare-sections__tab-name" data-section-id="<?=$arSection['ID'];?>">
								<?=$arSection['NAME'];?> &nbsp; &nbsp;<?=count($arSection["ITEMS"]);?>
							</span>
						</li>
					<?endforeach;?>
				</ul>
		...
		<div class="catalog-compare__top flexbox flexbox--row justify-content-between align-items-normal">
			...
			<div class="catalog-compare__switch form__check form__check--switch form__check--switch--secondary">
				<div class="onoff">
					<input class="onoff__input" type="checkbox" id="compare_diff" <?=($arResult['DIFFERENT'] ? 'checked' : '');?>>
					<label class="onoff__label font_13" for="compare_diff" class="muted font_13">
			...
			?>
			<span class="catalog-compare__clear font_13 color-theme-hover muted" onclick="CatalogCompareObj.MakeAjaxAction('<?=$GLOBALS['arTheme']['COMPARE_PAGE_URL']['VALUE'];?>?action=DELETE_FROM_COMPARE_RESULT&ID[]=<?=$arStr ?>', 'Y');">
				...
				<?foreach ($arResult['SECTIONS'] as $arSection):?>
					<div class="compare-sections__item <?=$arSection['ID'] === $activeTabId ? 'active' : '' ?>" data-section-id="<?=$arSection['ID'];?>">
									...
									<div class="catalog-block__wrapper height-100">
										<div class="catalog-block__item catalog_item bordered bg-theme-parent-hover border-theme-parent-hover js-notice-block" id="<?=$this->GetEditAreaId($arElement['ID']);?>">
											<div class="catalog-block__inner flexbox height-100">
												...
												// basket button
												$arItemIDs = COptimus::GetItemsIDs($arElement);
												$totalCount = COptimus::GetTotalCount($arElement, $arParams);
												$arAddToBasketData = COptimus::GetAddToBasketArray(
													$arElement,
													$totalCount,
												...
												<?$dataItem = COptimus::getDataItem($arElement);?>
													...
													<div class="catalog-block__info-bottom">
														<div class="line-block line-block--20 flexbox--wrap justify-content-between">
															...
															<div class="line-block__item catalog-block__info-bottom--margined js-popup-price catalog-block__info-price cost prices">
																...
																\Aspro\Functions\CAsproOptimusItem::showItemPrices($arParams, $arElement['PRICES'], '', $price_id, ($arParams["SHOW_DISCOUNT_PERCENT_NUMBER"] == "Y" ? "N" : "Y"));
																$frame->end();
																?>
															</div>
															...
															<div class="line-block__item catalog-block__info-bottom--margined catalog-block__info-btn">
																<div class="counter_wrapp">
																	<?if ($bShowCounterBlock):?>
																		<?=Aspro\Functions\CAsproOptimus::showItemCounter($arAddToBasketData, $arItemIDs, $arParams, $arItem);?>
																	<?endif;?>
									...
									<div class="bx_filtren_container ">
										<ul>
											<?if (!empty($arResult["ALL_FIELDS"])) {
												foreach ($arResult["ALL_FIELDS"] as $propCode => $arProp) {
													if (!isset($arResult['FIELDS_REQUIRED'][$propCode])){?>
														<li class="button button transparent <?=($arProp["IS_DELETED"] != "N" ? 'visible' : '');?> btn-xs">
						...
						<?if ($arShowFileds || $arShowOfferFileds || $arShowProps || $arShowOfferProps):?>
							<div class="catalog-compare__props-slider owl-carousel owl-theme" data-plugin-options='{"nav": false, "dots": false, "autoplay" : false, "autoplayTimeout" : "3000", "smartSpeed":500, "responsiveClass": true, "withSlide1": "catalog-compare__items", "rewind": true, "margin": -1, "responsive":{"0":{"items": 2},"768":{"items": 3},"992":{"items": 4}}}'>
								<?foreach ($arSection["ITEMS"] as $arElement){?>
									<div class="catalog-compare__item-props" data-id="<?=$arElement["ID"];?>">
										<?if ($arShowFileds):?>
											<?foreach ($arShowFileds as $code => $arProp):?>
												<div class="catalog-compare__prop-line 22">
										...
										<?if ($arShowOfferFileds):?>
											<?foreach ($arShowOfferFileds as $code => $arProp):?>
												<div class="catalog-compare__prop-line">
											...
											<?foreach ($arShowProps as $code => $arProp):?>
												<div class="catalog-compare__prop-line">
													...
													<?if ($arElement["DISPLAY_PROPERTIES"][$code]["DISPLAY_VALUE"]):?>
														<?$brandLinkProperty = (is_array($arElement["DISPLAY_PROPERTIES"][$code]["DISPLAY_VALUE"]) ? 
															implode(", ", $arElement["DISPLAY_PROPERTIES"][$code]["DISPLAY_VALUE"]) :
															str_replace(["#CURRENCY#", "href="], [$currency, "class=\"dark_link\" href="], $arElement["DISPLAY_PROPERTIES"][$code]["DISPLAY_VALUE"]));?>
										...
										<div class="catalog-small__item bordered flexbox flexbox--row height-100">
											...
											<div class="catalog-small__name">
												...
												// }
												?>
												<a href="<?=$arElement["DETAIL_PAGE_URL"];?>" class="dark_link switcher-title "><span class="lineclamp-3"><?=$name;?></span></a>
											</div>
										</div>
									</div>
								<?}
