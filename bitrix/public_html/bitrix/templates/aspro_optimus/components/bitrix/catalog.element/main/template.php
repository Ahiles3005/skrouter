...
<?if($arResult['SECTION']['ID'] == 56):?>
<style>

.price.only_price span {
color: #000;
}
</style>
<?endif;?>
				...
				<div class="prices_block">
					<div class="cost prices clearfix">
						...
						<?}else{?>
							...
							<?}?>
						<?}?>
<a class="btn btn-primary 2order" style="margin-top: 10px" data-url="<?=$arResult["DETAIL_PAGE_URL"]?>" data-name="<?=$arResult["NAME"]?>" data-id="<?=$arResult["ID"]?>" data-fancybox href="#modalZayavka">???????? ??????</a>
				...
				<?}else{?>
					...
					<div class="prices_block_empty"></div>
			<?}
		 
		 
		?>
				<div class="my_purchase_buttons">
					<div id="zayavka" class="my_button"><?=GetMessage("ZAYAVKA")?></div>
					<div id="rassrochka" class="my_button"><?=GetMessage("RASSROCHKA")?></div>
				</div>
				<div class="buy_block">
							...
							<table class="props_list">
								<?foreach($arResult["DISPLAY_PROPERTIES"] as $arProp):?>
									<?if(!in_array($arProp["CODE"], array("SERVICES", "BRAND", "HIT", "RECOMMEND", "NEW", "STOCK", "VIDEO", "VIDEO_YOUTUBE", "CML2_ARTICLE"))):?>
										...
										<?if ($arProp["CODE"]=='arenda') continue; ?>
							...
							foreach($arFiles as $arItem):?>
								...
								<div class="file_type clearfix <?=$arFile["TYPE"];?>">
									...
									<div class="description">
										<a class="file__btn" target="_blank" href="<?=$arFile["SRC"];?>"><?/*=$arFile["DESCRIPTION"];*/?><?=GetMessage("DOWNLOAD")?>
										<svg class="file__icon" width="27" height="30" viewBox="0 0 27 30" fill="none" xmlns="http://www.w3.org/2000/svg">
  <path d="M13.5 24.825L5.25 15H21.75L13.5 24.825ZM10.5 0H16.5V3H10.5V0ZM10.5 4.5H16.5V7.5H10.5V4.5Z" fill="white" />
  <path d="M10.5 9H16.5V17.25H10.5V9ZM0 27H27V30H0V27Z" fill="white" />
</svg>
									</a>
				...
				<?endif;?>
			<? if($arResult["PROPERTIES"]["arenda"]["VALUE"]):?>
				<div class="arenda"><?=htmlspecialcharsBack($arResult["PROPERTIES"]["arenda"]["VALUE"]["ТЕXT"])?></div>
			<?endif;?>
...
<div class="tabs_section">
	...
	<ul class="tabs1 main_tabs1 tabs-head">
		...
		<?endif;?>
		<li class="<?=(!($iTab++) ? ' current' : '')?>">
			<span><?=GetMessage("MY_DELIVERY");?></span>
		</li>
		<li class="<?=(!($iTab++) ? ' current' : '')?>">
			<span><?=GetMessage("MY_PAYMENT");?></span>
		</li>
		<li class="<?=(!($iTab++) ? ' current' : '')?>">
			<span><?=GetMessage("MY_WARRANTY");?></span>
		</li>
...
*/?>
		...
		<li class="<?=(!($iTab++) ? ' current' : '')?>">
			<?$APPLICATION->IncludeFile(SITE_DIR."include/my_delivery.php", array(), array("MODE" => "html", "NAME" => GetMessage('MY_DELIVERY')));?>
		</li><li class="<?=(!($iTab++) ? ' current' : '')?>">
			<?$APPLICATION->IncludeFile(SITE_DIR."include/my_payment.php", array(), array("MODE" => "html", "NAME" => GetMessage('MY_PAYMENT')));?>
		</li><li class="<?=(!($iTab++) ? ' current' : '')?>">
			<?$APPLICATION->IncludeFile(SITE_DIR."include/my_warranty.php", array(), array("MODE" => "html", "NAME" => GetMessage('MY_WARRANTY')));?>
		</li>
...
<div class="item_main_info__slider">
<?
	GLOBAL $arrFilterSection;

	  $arrFilterSection = array(   '!ID' => $arResult["ID"]);
...
<div id="my_zayavka_popup" class="my_order_popup">
	<div class="form inline ZAYAVKA">
		<!--noindex-->
		<div class="my_closer"><img src="https://skrouter.ru/bitrix/templates/aspro_optimus/images/closer.svg"></div>
		<div class="form_head">
			<h4 class="my_order_title"><?=GetMessage("ZAYAVKA")?></h4>
		</div>
			<form id="ZAYAVKA" action="" method="POST" enctype="multipart/form-data">
			<div class="form_body">
				<div class="form-control">
				<label><span><?=GetMessage("ORM_NAME")?>&nbsp;<span class="star">*</span></span></label>
				<input id="z_name" type="text" class="inputtext" required="" value="" minlength="3" maxlength="50"></div>
				<div class="form-control">
					<label><span><?=GetMessage("ORM_PHONE")?>&nbsp;<span class="star">*</span></span></label>
					<input id="z_phone" type="text" class="phone" required="" value="">			
				</div>
				<div class="form-control">
					<label><span><?=GetMessage("ORM_EMAIL")?>&nbsp;<span class="star">*</span></span></label>
					<input id="z_email" type="email" class="inputtext" required="" value="">			
				</div>
				<input id="z_tovar" type="text" class="inputtext" required="" value="ID = <?=$arResult['ID']?>; url = <?=$APPLICATION->GetCurDir()?>" aria-required="true" style="display:none">
				<div class="clearboth"></div>
				<div class="clearboth"></div>
			</div>
			<div class="form_footer">
				<div class="licence_block filter label_block">
				<input type="checkbox" id="licenses_popup_ZAYAVKA" name="licenses_popup" required="" value="Y">
				<label for="licenses_popup_ZAYAVKA">
					<?=GetMessage("ORM_SOGL_1")?><a href="/include/licenses_detail.php" target="_blank"><?=GetMessage("ORM_SOGL_2")?></a>				</label>
				</div>
					<input id="z_button" type="submit" class="button medium" value="<?=GetMessage("ORM_SEND")?>">
				</div>
		</form>	<!--/noindex-->
	</div>
	<?/*$APPLICATION->IncludeComponent(
        ...
        $.ajax({
            ...
            type: "post", // метод передачи
             data: { // что отправляем
                 ...
                 "tovar": tovar,
				 "email": email,
				 "type": '<?=GetMessage("ORM_Z")?>',
             },
            error: function () {
                $("#z_button").val("Произошла ошибка!");
            },
            beforeSend: function () {
                $("#z_button").val("<?=GetMessage('ORM_SENDING')?>");
            },
            success: function () {
                $('#ZAYAVKA').html("<span style='font-size: 18px;text-align: center;display: block;margin: 3rem 1rem;line-height: 26px;'><?=GetMessage('ORM_SENT')?></span>");
            }
        });
    }); 
</script>
<div id="my_rassrochka_popup" class="my_order_popup">
	<div class="form inline RASSROCHKA">
		<!--noindex-->
		<div class="my_closer"><img src="https://skrouter.ru/bitrix/templates/aspro_optimus/images/closer.svg"></div>
		<div class="form_head">
			<h4 class="my_order_title"><?=GetMessage("ORM_R")?></h4>
		</div>
			<form id="RASSROCHKA" action="" method="POST" enctype="multipart/form-data">
			<div class="form_body">
				<div class="form-control">
				<label><span><?=GetMessage("ORM_NAME")?>&nbsp;<span class="star">*</span></span></label>
				<input id="r_name" type="text" class="inputtext" required="" value="" minlength="3" maxlength="50"></div>
				<div class="form-control">
				<label><span><?=GetMessage("ORM_PHONE")?>&nbsp;<span class="star">*</span></span></label>
				<input id="r_phone" type="text" class="phone" required="" value="">			</div>
				<div class="form-control">
					<label><span><?=GetMessage("ORM_EMAIL")?>&nbsp;<span class="star">*</span></span></label>
					<input id="r_email" type="email" class="inputtext" required="" value="">			
				</div>
				<input id="r_tovar" type="text" class="inputtext" required="" value="ID = <?=$arResult['ID']?>; url = <?=$APPLICATION->GetCurDir()?>" aria-required="true" style="display:none">
				<div class="clearboth"></div>
				<div class="clearboth"></div>
			</div>
			<div class="form_footer">
				<div class="licence_block filter label_block">
				<input type="checkbox" id="licenses_popup_RASSROCHKA" name="licenses_popup" required="" value="Y">
				<label for="licenses_popup_RASSROCHKA">
					<?=GetMessage("ORM_SOGL_1")?><a href="/include/licenses_detail.php" target="_blank"><?=GetMessage("ORM_SOGL_2")?></a>				</label>
				</div>
					<input id="z_button" type="submit" class="button medium" value="<?=GetMessage("ORM_SEND")?>">
				</div>
...
<script>
	$("#RASSROCHKA").on('submit', function (e) {
        // собираем данные с формы
        e.preventDefault();
        var name = $('#r_name').val();
        var phone = $('#r_phone').val();
        var email = $('#r_email').val();
        var tovar = $('#r_tovar').val();
        // отправляем данные
        $.ajax({
            url: "/send.php", // куда отправляем
            type: "post", // метод передачи
            ...
            success: function () {
                $('#RASSROCHKA').html("<span style='font-size: 18px;text-align: center;display: block;margin: 3rem 1rem;line-height: 26px;'><?=GetMessage('ORM_SENT')?></span>");
            }
        });
    }); 
</script>
